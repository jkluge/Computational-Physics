...
# z[2] decreasing with t for  the event to be true, the ball should be on its way down
position.direction = -1
position.terminal = True  # integration terminates at event
for i in range(1, 90):
    sol = integrate.solve_ivp(traj, [0, 20], [
                              0, v * np.cos(math.radians(i)), 0, v * np.sin(math.radians(i))], events=position)
    if max_throw < sol.y[0][-1]:
        best_theta = i
        max_t = float(sol.t_events[0][0])
        max_throw = sol.y[0][-1]

sol = integrate.solve_ivp(traj, [0, max_t], [0, v * np.cos(math.radians(best_theta)),
